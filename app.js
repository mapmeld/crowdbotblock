    ...
    blockcode.blockcode.findOne({ status: 'cue' }).sort('updated').exec(function(err, doc){
      ...
      if(doc.xml && doc.xml.length){
        ...
        if(io && io.sockets){
          ...
          io.sockets.emit('newprogram', { wiring: replaceAll(replaceAll(doc.wiring, "<", "&lt;"), ">", "&gt;"), name: doc.name, id: doc._id });
        }
        ...
        // return Wiring code to host
        res.send({ _id: doc._id, wiring: doc.wiring });
      }
    });
  });
